...
contract LiquidityPool is LpToken {
  ...
  function depoisit(uint amount) external {
  ...
  function _distributeRewards(address beneficiary) internal {
    ...
    if(block.number - checkpoint > 0) {
      uint distributuinAmount = balanceOf(beneficiary) * (block.number - checkpoint * REWARD_PER_BLOCK);

      governanceToken.mint(beneficiary, distributuinAmount);
